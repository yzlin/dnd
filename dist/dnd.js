...
`;
  ...
  const getStylesRoot = stylesInsertionPoint => {
    const stylesRoot = stylesInsertionPoint || document.querySelector('head');
    !stylesRoot ? invariant$1(false, 'Cannot find the head or root to append a style to')  : void 0;
    return stylesRoot;
  };
  ...
  function queryElements(ref, selector, filterFn) {
    const rootNode = ref && ref.getRootNode();
    const documentOrShadowRoot = rootNode && rootNode.querySelectorAll ? rootNode : document;
  ...
  function findDragHandle(contextId, draggableId, ref) {
    ...
    const handle = queryElements(ref, selector, el => {
...
`;
  ...
  function findDraggable(contextId, draggableId, ref) {
    ...
    const draggable$1 = queryElements(ref, selector, el => {
  ...
  function tryStart({
    ...
    const el = findDraggable(contextId, entry.descriptor.id, getEventTargetRoot(sourceEvent));
    if (!el) {
  ...
  function App(props) {
    ...
    const styleMarshal = useStyleMarshal(contextId, nonce, props.stylesInsertionPoint);
  ...
  function useValidation$1(props, contextId, getRef) {
    useDevSetupWarning(() => {
      ...
      if (props.isEnabled) {
        !findDragHandle(contextId, id, getRef()) ? "development" !== "production" ? invariant$1(false, `${prefix(id)} Unable to find drag handle`) : invariant$1(false) : void 0;
      }
    });
  }
